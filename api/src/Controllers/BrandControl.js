const { Brand, Article} = require('../db');
const allArt = [
  {
    "id": 1,
...
const getArticle = async () => {
  ...
  const api2 = apiDB.map(el => {
    ...
    let BrandMap = apiDB.map(el=>{
      return [el.detail.marca,el]
     });
    var BrandMapArr = new Map(BrandMap); // Pares de clave y valor
    let unicos = [...BrandMapArr.values()]; // Conversión a un array

  return (unicos);
};
const getAticleByName = async (req, res, next) => {
    let getBrandTotal = await getArticle();
    res.status(200).send(getBrandTotal);
};

async function loadBrandDb() {
  // Verificamos si los artículos ya fueron creados
  let getBrandTotal = await getArticle();
  const allArticles = await Brand.findAll()
  if (allArticles.length > 0) {
    console.log("Marca cargados previamente")
    return
  }
  
  getBrandTotal.forEach(async el => {
    Brand.findOrCreate({
      where:{marca: el},
    })
  })
